...
<ClieLvOnly><CodePreview>
<textarea lang="ts">
LvMessage.info("这是一条普通的提示信息")
</textarea>
<template #preview>
<Button @click="Message.info('这是一条普通的提示信息')">显示信息</Button>
</template>
</CodePreview></ClieLvOnly>

## 不同状态

用来显示「成功、警告、消息、错误、加载」类的操作反馈。
...
<Button @click="Message.loading('这是一条加载的提示信息')" class="ml-10">显示加载</Button>
</template>
</CodePreview></ClieLvOnly>
...
<template #preview>
<Button @click="Message.info({ content: '这是一条提示信息', duration: 5000 })">持续5s</Button>
</template>
</CodePreview></ClieLvOnly>

## 更新消息

通过 `updateMessage` 方法可以更新指定 `id` 的消息内容。
...
<template #preview>
<Button @click="handleShowLoading">加载</Button>
<Button @click="loadingSuccess" class="ml-10">加载成功</Button>
<Button @click="loadingError" class="ml-10">加载失败</Button>
</template>
</CodePreview></ClieLvOnly>
...
| 名称 | 参数 | 说明 | 返回类型 |
| --- | --- | --- | --- |
| `info` | `MessageOption \| string` | 显示普通提示信息, 参数为配置项, 返回消息 `id` | `string` |
| `success` | `MessageOption \| string` | 显示成功提示信息 | `string` |
| `warn` | `MessageOption \| string` | 显示警告提示信息 | `string` |
| `error` | `MessageOption \| string` | 显示错误提示信息 | `string` |
| `close` | `id: string` | 手动关闭消息 | - |
| `closeAll` | - | 手动关闭所有消息 | - |
| `update` | `(id: string, message: MessageOption) => void` | 更新指定 `id` 的消息内容 | - |
...
| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `content` | `string` \| `VNode` | - | 消息正文内容 |
| `duration` | `number` | `3000` | 消息显示的持续时间，单位为毫秒。如果设置为 `0`，则不会自动关闭 |
| `type` | `string` | `info` | 消息类型，可选值为 `success`、`warning`、`error`、`info`、`loading` |
